...
function escolhaPlayer2(){//definir o bot
...
function telaInicial(){ // fazer a troca de "paginas", mudando os displays dos 2 divs 
...
function checkjogo(id) {
    ...
    if(anulado) { // impedir que possa clicar em celulas vazias após o fim do jogo
        return false;
    }
    ...
    if (opt == "transp.png") {
        ...
        if(cpu && player == bot){ // fazer o som do bot ser acionado qnd for sua vez
    ...
    if (numJog >= 9) {
        ...
        if (player == bot) { //alternância entre player e bot
            player = bot1;
        } else {
            player = bot;
               }
...
if (cpu && player == bot) {
           setTimeout(() => {checkjogo(jogoDoPc())}, 1000)//tempo de 1s para o bot jogar
                                  }
